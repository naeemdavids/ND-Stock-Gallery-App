...
import type { SearchImageResults } from "@/models/Images";
...
export default function nextAndPrevPages(images: SearchImageResults) {
  ...
  const totalPages =
    images.total_results % images.per_page // Check if there's a remainder when dividing total results by per_page.
      ? Math.ceil(images.total_results / images.per_page) // If so, round up to the nearest integer.
      : images.total_results / images.per_page + 1; // Otherwise, add 1 for the total number of pages.
