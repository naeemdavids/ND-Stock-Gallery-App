// Import the ImagesGallery component, which is used to display a gallery of images.
...
export function generateMetadata({ params: { pageParams } }: Props) {
  // Extract the search topic from the first element of pageParams (default to "curated" if not provided).
  const searchTopic = pageParams?.[0] ?? "curated";
...
export default function SearchResults({ params: { pageParams } }: Props) {
  // Extract the search topic from the first element of pageParams, defaulting to "curated" if no topic is specified.
  const searchTopic = pageParams?.[0] ?? "curated";

  // Extract the page number from the second element of pageParams, defaulting to "1" if not specified.
  const page = pageParams?.[1] ?? "1";

  // Render the ImagesGallery component, passing the search topic and page as props.
  return <ImagesGallery searchTopic={searchTopic} page={page} />;
}
