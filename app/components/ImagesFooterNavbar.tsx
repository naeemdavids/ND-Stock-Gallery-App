...
export default function ImagesFooterNavbar({
  ...
  const nextPageButtonArea = nextPage ? (
    <Link
      href={
        searchTopic === "curated" // Check if the topic is "curated" (default topic).
          ? `/results/images/curated/${nextPage}` // If curated, use the curated results URL.
          : `/results/images/${searchTopic}/${nextPage}` // For custom search topics, construct URL with the topic and nextPage.
      }
  ...
  const prevPageButtonArea = prevPage ? (
    <>
      ...
      <Link
        href={`/results/images/${searchTopic}/${prevPage}`} // Construct the URL for the previous page.
      ...
      {pageNumbers.map((pageNumber) =>
        ...
        page && pageNumber === parseInt(page) ? (
          ...
          <Link
            key={pageNumber}
            href={`/results/images/${searchTopic}/${pageNumber}`}
          >
            {`#${pageNumber}`}{" "}
